...
import { ConvertDollar, convertToman, formatNumber, separtment } from "../logic/logic"
...
function CurrencyConvert() {
    ...
    const dollar: number = data?.usd.value ?? 0
  ...
  return (
    <div className=" container md:flex-col m-auto p-2 w-full h-full flex flex-col justify-between items-center gap-4 bg-blue-50 shadow-2xl">
     ...
     <div className="  flex  w-full h-40  p-4 mx-4 justify-between items-center  gap-4 bg-blue-100 shadow-2xl rounded-2xl">
      ...
      <p className="flex items-center text-xl font-semibold gap-2">{formatNumber(dollar)}<img src="./toman.png" alt="Toman"  className="w-5 h-5"/>: قیمت روز دلار</p>
      </div>
...
/>        <div className="flex gap-2 items-center justify-center">
        <button onClick={() => convertToman({money, dollar, setMoney,setCurrency})} className=" rounded-2xl  w-40 h-20 hover:bg-black hover:text-white hover:w-50 bg-blue-600">تبدیل به تومان </button>
        <button onClick={() => ConvertDollar({money, dollar, setMoney,setCurrency})} className=" rounded-2xl w-40 h-20 hover:bg-black hover:text-white hover:w-50 bg-blue-600">تبدیل به دلار</button>
        </div>
